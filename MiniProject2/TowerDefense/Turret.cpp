...
Turret::Turret(std::string imgTurret, float x, float y, float radius, int price, float coolDown, int hp, bool isBoom) :
	Sprite(imgTurret, x, y), price(price), coolDown(coolDown), hp(hp), isBoom(isBoom){
...
void Turret::Draw() const {
	...
	if(isBoom)
		al_draw_circle(Position.x, Position.y, CollisionRadius * 10, al_map_rgb(255, 0, 0), 2);
}
